...
func TestGetFileDescriptorProtoByFilename(t *testing.T) {
	...
	fileProto, err = GetFileDescriptorProtoByFilename(filename)
	if err != nil {
	...
	if want := "gogo.proto"; fileProto.GetName() != want {
		t.Errorf("file name got %q\nwant %q", fileProto.GetName(), want)
	}

	if want := "gogoproto"; fileProto.GetPackage() != want {
		t.Errorf("file package got %q\nwant %q", fileProto.GetPackage(), want)
	}

	if want := "github.com/gogo/protobuf/gogoproto"; fileProto.GetOptions().GetGoPackage() != want {
		t.Errorf("go package option got %q\nwant %q", fileProto.GetOptions().GetGoPackage(), want)
	}

	{
		extension := fileProto.GetExtension()[0]
